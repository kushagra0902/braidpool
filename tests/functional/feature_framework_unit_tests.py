...
from test_runner import schedule_tests, parse_args, tail_file
...
def test_network_config_defaults() -> None:
    ...
    assert cfg.braidpool_bind_ports == []
    assert cfg.braidpool_rpc_ports == []
    assert cfg.braidpool_stratum_ports == []


def test_network_config_lists_are_independent() -> None:
    first = NetworkConfig()
    second = NetworkConfig()
    first.braidpool_rpc_ports.append(12345)
    assert_equal(second.braidpool_rpc_ports, [])
...
def test_bitcoin_node_args_construction() -> None:
    ...
    with tempfile.TemporaryDirectory() as td:
        config = NetworkConfig(
            ...
            bitcoin_cli_path=Path("/mock/bitcoin-cli"),
        )
...
def test_test_node_port_allocation() -> None:
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        assert node0.rpc_port != node1.rpc_port
        assert node0.p2p_port != node1.p2p_port
        assert node0.stratum_port != node1.stratum_port
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        class MockTestNode:
            ...
            def start(self, extra_args):
                if self.index > 0:
                    captured_extra_args.extend(extra_args)
        ...
        assert any("--addnode" in arg for arg in captured_extra_args)
...
TESTS = [
    ...
    test_network_config_lists_are_independent,
